...
:::caution Gmail requires OAuth
Google has disabled password-based (Basic auth) IMAP/SMTP access for Google Workspace accounts,
and app passwords are being phased out. For Gmail, use the OAuth2 auth type with an access token
(via the [`oauth2` generator](/generators/oauth2)). XOAUTH2 over SMTP remains officially supported;
see [Google's XOAUTH2 documentation](https://developers.google.com/workspace/gmail/imap/xoauth2-protocol).
:::
