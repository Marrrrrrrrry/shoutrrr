...
:::tip 相关指南
本页是 URL 格式与参数的参考文档。
获取 *token* 和 *channel* 的图文步骤参见[获取 Slack Token 指南](/zh/guides/slack/)。
:::

:::caution 旧版 Webhook 令牌已被 Slack 弃用
Slack 已弃用旧版 custom integration 的 incoming webhook：目前仍可使用，
但没有移除时间表、可能随时失效。优先使用 Bot API 令牌
（`xoxb`，见[Token 指南](/zh/guides/slack/)）或通过 Slack App 创建的 incoming webhook。
:::
